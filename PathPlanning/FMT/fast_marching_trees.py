...
class FMT:
    ...
    def Planning(self):
        ...
        # 采样半径是会随着sample_numbers采样个数增大而减小的
        rn = self.search_radius * math.sqrt((math.log(n) / n))
        # 已经加入的树的节点，并且该节点已经不会再产生新的连接
        Visited = []

        while z is not self.x_goal:
            # 这个就是为了装z周围的x中成功获得连接，可以成功加入树里面的节点
            V_open_new = set()
            # 先找到Z附近的采样点
        ...
        # Visited[1: len(Visited)]就是有的节点个数
    ...
    # def Cost(self, x_start, x_end): 这里的检测跟陆lazy的原则是不需要的，我们只需要最后一行的return语句计算对应的距离就行
    #     # 根据上面的代码,我觉得这个地方可以直接不要????
    ...
    def SampleFree(self):
        ...
        ind = 0
        while ind < n:
            # self.x_range这个就是对应的地图的x范围,self.y_range对应的地图的y范围
